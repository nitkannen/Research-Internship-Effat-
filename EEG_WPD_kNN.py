...
mat_contents = sio.loadmat('AS_BONN_ALL_EEG_DATA_1024.mat')
sorted(mat_contents.keys())

EpilepticZone_Interictal=mat_contents['EpilepticZone_Interictal']
Epileptic_Ictal=mat_contents['Epileptic_Ictal']
NonEpilepticZone_Interictal=mat_contents['NonEpilepticZone_Interictal_']
Normal_Eyes_Closed=mat_contents['Normal_Eye_Closed']
Normal_Eyes_Open=mat_contents['Normal_Eyes_Open']
...
for i in range(numcols):
    wp= pywt.WaveletPacket(Normal_Eyes_Open[:,i], db1, mode='symmetric', maxlevel=6)
...
for i in range(numcols, 2*numcols):
    wp= pywt.WaveletPacket(EpilepticZone_Interictal[:,i-numcols], db1, mode='symmetric', maxlevel=6)
    Extracted_Features[i,0]=sp.mean(abs(wp['a'].data))
    Extracted_Features[i,1]=sp.mean(abs(wp['aa'].data))
    Extracted_Features[i,2]=sp.mean(abs(wp['aaa'].data))
    Extracted_Features[i,3]=sp.mean(abs(wp['aaaa'].data))
    ...
    temp_list.append("INTERICTAL")

"""ICTAL EEG SIGNAL"""  
for i in range(2*numcols, 3*numcols):
    wp= pywt.WaveletPacket(Epileptic_Ictal[:,i-2*numcols], db1, mode='symmetric', maxlevel=6)
    Extracted_Features[i,0]=sp.mean(abs(wp['a'].data))
    Extracted_Features[i,1]=sp.mean(abs(wp['aa'].data))
    Extracted_Features[i,2]=sp.mean(abs(wp['aaa'].data))
    Extracted_Features[i,3]=sp.mean(abs(wp['aaaa'].data))
